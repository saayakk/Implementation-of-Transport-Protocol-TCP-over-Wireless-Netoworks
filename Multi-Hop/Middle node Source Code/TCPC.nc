...
implementation
{
	...
	void sendData()
	{
		...
		tcpmsg->seqNo = seqNo;		//Assigning manipulated seqNo
	...
	event message_t * radioReceive.receive(message_t *msg, void *payload, uint8_t len)
	{
		...
		if(tcpmsg->dip == 15)		// If I am destination, I run TCP
		{
			...
			if(tcpmsg->type == 1)	//SYN_ACK received
			{
				synackFlag = 1;
			...
			if(tcpmsg->type == 7)	//ACK received
			{
				...
				{					
					counter += 1;
					seqNo += 1;		//seqNo = tcpmsg->seqNo;
				}
				if(seqNo <= (sz-1))	
				{	
					sendToPC();		
					call timer2.startOneShot(1000);	//Call to send Data
				}
				else
				{
					sendFIN();
		...
		else		// If I am not the destination, I forward to the actual destination  (assumed to be next hop)
		{	
